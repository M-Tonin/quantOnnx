...
from math import ceil, e as exponential
...
def new_onnx_filename(thepath, only_name):
    ...
    for i in range (24,1,-1):
        chans.append(i)
        st = ""
        st = thepath + only_name_file + "_enc_" + str(i) + "channels_bits.onnx"
        liname.append(st)
...
def enconder_muLaw(valor:np.float32,Xmax, micro):
    cara = Xmax * ( np.log(1 + micro * (np.abs(valor)/Xmax ))/(np.log(1+micro)) ) * np.sign(valor)
    quant = (cara + 1) / 2 * micro + 0.5
    quant = quant.astype(int)
    return quant
	
def decoder_muLaw(y,Xmax, micro):
	y = y.astype(np.float32)
	y = 2 * (y / micro) - 1
	x = (Xmax / micro) * (exponential ** ((np.log(1+micro)*abs(y))/Xmax ) - 1) * np.sign(y)
	return x
	

def muLaw(valor:np.float32,Xmax,micro):
	y_val = enconder_muLaw(valor,Xmax, micro)
	new_val = decoder_muLaw(y_val,Xmax, micro)
	return new_val, y_val

def muLawEnconding(valor_pass:np.float32, abs_maxi, files_onnx, channel_format):
    list_int_quant = []
    list_float_desquant = []
    #print(valor_pass)
    #abs_maxi = maxi if (maxi > abs(mini)) else abs(mini)
    for channel in channel_format:
        
        if(math.isnan(valor_pass)):
            printLog("Problema - IsNan Encontrado!")
        xq, q_int = muLaw(valor_pass,abs_maxi,channel)
        #print(f'do valor {valor_pass} ({q_int} = {xq})')
        list_int_quant.append((q_int))
    ...
    val_23b = int(np.mean(list_int_quant))
    float_back = np.mean(list_float_desquant)
...
loggname = 'LogMiddle_'+date_time+'.txt'
...
new_onnxs_diretory = raiz+"onnxs_encoded_muLaw/"
if not os.path.exists(new_onnxs_diretory):
    os.makedirs(new_onnxs_diretory)

names_file, canais = new_onnx_filename(new_onnxs_diretory,only_name_file)
...
for chan in canais:
    if abs_maxi > 1.0:
        canais = int(ceil(((2 ** (chan - 1)) - 1) * 1/(1 + np.log(abs_maxi))))
    else :
        canais = 2 ** (chan - 1) - 1
    if canais < 1.0 :
            canais = 1
    canais_value.append(canais)
...
with open(main_dot_onnx_path, "rb") as f:
    ...
    while byte:
        ...
        if  i == act_start:
            ...
            numc_f = (unpack("<f",byte_c))[0]
            #print(numc_f)
            new_value, new_float = muLawEnconding(numc_f,abs_maxi,onnx_files, canais_value)
            printLog("[",hex(i-3),"]",hex(bytec_int), "float rep = ", numc_f, " quantized ", {new_float} )
            ...
            for cont_qnt in range (act_quant - 1):
                ...
                num_f = (unpack("<f",byte_4))[0]
                #print(num_f)
                new_value, new_float = muLawEnconding(num_f,abs_maxi,onnx_files, canais_value)
                #printLog("[",hex(i-3),"]",hex(bytec_int), "float rep = ", num_f, " quantized ", {new_float} )
                i = i + 4
            r_size = cont_qnt + 2
